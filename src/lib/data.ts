export const heroParallax = [

  {
    title: "Algochurn",
    link: "/",
    thumbnail: "politics3.png",
  },
  {
    title: "Cursor",
    link: "/",
    thumbnail: "org1.png",
  },
  {
    title: "Moonbeam",
    link: "/",
    thumbnail: "politics.png",
  },
  {
    title: "Pixel Perfect",
    link: "/",
    thumbnail: "business1.png",
  },

  {
    title: "Aceternity UI",
    link: "/",
    thumbnail: "business2.png",
  },
  {
    title: "Tailwind Master Kit",
    link: "/",
    thumbnail: "politics2.png",
  },
  {
    title: "Editrix AI",
    link: "/",
    thumbnail: "org3.png",
  },
  {
    title: "Rogue",
    link: "/",
    thumbnail: "business1.png",
  },

  {
    title: "Editorially",
    link: "/",
    thumbnail: "business2.png",
  },
  {
    title: "Golden Bells Academy",
    link: "/",
    thumbnail: "business4.png",
  },
  {
    title: "SmartBridge",
    link: "/",
    thumbnail: "politics4.png",
  },
  {
    title: "Renderwork Studio",
    link: "/",
    thumbnail: "org4.png",
  },
  {
    title: "Creme Digital",
    link: "/",
    thumbnail: "business3.png",
  },
  {
    title: "Invoker Labs",
    link: "/",
    thumbnail: "politics6.png",
  },
  {
    title: "E Free Invoice",
    link: "/",
    thumbnail: "org5.png",
  },
];

export const demoPrices = [
  {
    id: "price_trial",
    name: "Bronze",
    speciality: "Basic Plan",
    description: "Explore all basic features with bronze plan.",
    features: [
      "1 App User (For Auto SMS)",
      "Contact Action Buttons",
      "Social Media Integration",
      "News Section",
      "Photo Gallery",
      "Video Gallery",
      "Downloads Section",
      "Testimonials",
    ],
    notFeature: [
      "Enquiry Form",
      "Multi Language Support"
    ],
    monthlyPrice: 9000,
    yearlyPrice: 9000,
    isMostPopular: false,
    showPrices: false,
  },
  {
    id: "price_basic",
    name: "Silver",
    speciality: "Startup Special⚡",
    description: "A basic plan for startups and individual users.",
    features: [
      "3 App User (For Auto SMS)",
      "Contact Action Buttons",
      "Social Media Integration",
      "News Section",
      "Photo Gallery",
      "Video Gallery",
      "Downloads Section",
      "Testimonials",
      "Enquiry Form"
    ],
    notFeature: [
      "Multi Language Support"
    ],
    monthlyPrice: 19000,
    yearlyPrice: 19000,
    isMostPopular: false,
    showPrices: false,
  },
  {
    id: "price_standard",
    name: "Gold",
    speciality: "Growing Businesses⚡",
    description: "A gold plan for growing businesses.",
    features: [
      "7 App User (For Auto SMS)",
      "Contact Action Buttons",
      "Social Media Integration",
      "News Section",
      "Photo Gallery",
      "Video Gallery",
      "Downloads Section",
      "Testimonials",
      "Enquiry Form",
      "Multi Language Support"
    ],
    notFeature: [],
    monthlyPrice: 29000,
    yearlyPrice: 29000,
    isMostPopular: true,
    showPrices: false,
  },
  {
    id: "price_politician_premium",
    name: "Platinum",
    speciality: "Politicians Special ✨",
    description: "Designed for political leaders to maximize their reach.",
    features: [
      "Unlimited App User (For Auto SMS)",
      "Contact Action Buttons",
      "Social Media Integration",
      "News Section",
      "Photo Gallery",
      "Video Gallery",
      "Downloads Section",
      "Testimonials",
      "Enquiry Form",
      "Multi Language Support"
    ],
    notFeature: [],
    monthlyPrice: "✨",
    yearlyPrice: "****",
    isMostPopular: false,
    showPrices: false,
  },
];

export const COMPANY = [
  {
    name: "Home",
    link: "/",
  },
  {
    name: "Features",
    link: "#features",
  },
  {
    name: "Pricing",
    link: "#pricing",
  },
  {
    name: "About",
    link: "#about",
  },
  {
    name: "Themes",
    link: "/themes",
  },
];

export const HELP = [
  {
    name: "Terms & Condition",
    link: "./Terms and Conditions - Sampark Bharat.pdf",
  },
  {
    name: "Privacy Policy",
    link: "./Privacy Policy - Sampark Bharat.pdf",
  }
];

export const TECHS = [
  {
    name: "Blog 1",
    link: "./Blog 1.pdf",
  },
  {
    name: "Blog 2",
    link: "./Blog 2.pdf",
  },
  {
    name: "Blog 3",
    link: "./Blog 3.pdf",
  },
  {
    name: "Blog 4",
    link: "./Blog 4.pdf",
  },
];

// Define available themes with Tailwind classes
// export const solidThemes = {
//   Solid1: {
//     background: 'bg-orange-400',
//     textBackground: 'text-orange-400',
//     textPrimary: 'text-gray-700',
//     textSecondary: 'text-gray-600',
//   },
//   Solid2: {
//     background: 'bg-[#DE2695]',
//     textBackground: 'text-[#DE2695]',
//     textPrimary: 'text-white',
//     textSecondary: 'text-white',
//   },
//   Solid3: {
//     background: 'bg-sky-400',
//     textBackground: 'text-sky-400',
//     textPrimary: 'text-white',
//     textSecondary: 'text-white',
//   },
//   Solid4: {
//     background: 'bg-slate-900',
//     textBackground: 'text-slate-900',
//     textPrimary: 'text-slate-200',
//     textSecondary: 'text-slate-300',
//   },
//   Solid5: {
//     background: 'bg-[#c7dfef]',
//     textBackground: 'text-blue-600',
//     textPrimary: 'text-gray-700',
//     textSecondary: 'text-blue-600',
//   },
//   Solid7: {
//     background: 'bg-[#4B0082]', // Indigo
//     textBackground: 'text-[#4B0082]',
//     textPrimary: 'text-white',
//     textSecondary: 'text-gray-200',
//   },
//   Solid12: {
//     background: 'bg-[#20B2AA]', // Light Sea Green
//     textBackground: 'text-[#20B2AA]',
//     textPrimary: 'text-white',
//     textSecondary: 'text-gray-200',
//   },
// };

export const solidThemes = {
  Solid1: {
    background: 'bg-orange-400',
    textBackground: 'text-orange-400',
    textPrimary: 'text-gray-700',
    textSecondary: 'text-gray-600',
  },
  Solid2: {
    background: 'bg-[#DE2695]',
    textBackground: 'text-[#DE2695]',
    textPrimary: 'text-white',
    textSecondary: 'text-white',
  },
  Solid3: {
    background: 'bg-sky-400',
    textBackground: 'text-sky-400',
    textPrimary: 'text-white',
    textSecondary: 'text-white',
  },
  Solid4: {
    background: 'bg-slate-900',
    textBackground: 'text-slate-900',
    textPrimary: 'text-slate-200',
    textSecondary: 'text-slate-300',
  },
  Solid5: {
    background: 'bg-[#c7dfef]',
    textBackground: 'text-blue-600',
    textPrimary: 'text-gray-700',
    textSecondary: 'text-blue-600',
  },
  Solid6: {
    background: 'bg-[#2E4C6D]',  // Rich dark blue
    textBackground: 'text-[#2E4C6D]',
    textPrimary: 'text-white',
    textSecondary: 'text-gray-300',
  },
  Solid7: {
    background: 'bg-[#8C5B4A]',  // Warm terracotta
    textBackground: 'text-[#8C5B4A]',
    textPrimary: 'text-white',
    textSecondary: 'text-gray-300',
  },
  Solid8: {
    background: 'bg-[#a71f13]',
    textBackground: 'text-[#a71f13]',
    textPrimary: 'text-white',
    textSecondary: 'text-gray-300',
  },
  Solid9: {
    background: 'bg-[#a7382d]',
    textBackground: 'text-[#a7382d]',
    textPrimary: 'text-white',
    textSecondary: 'text-gray-400',
  },
  Solid10: {
    background: 'bg-[#6a994e]',
    textBackground: 'text-[#6a994e]',
    textPrimary: 'text-white',
    textSecondary: 'text-gray-200',
  },
  Solid11: {
    background: 'bg-[#ff6700]',
    textBackground: 'text-[#ff6700]',
    textPrimary: 'text-white',
    textSecondary: 'text-gray-400',
  },
  Solid12: {
    background: 'bg-[#0a9396]',
    textBackground: 'text-[#0a9396]',
    textPrimary: 'text-white',
    textSecondary: 'text-gray-300',
  },
  Solid13: {
    background: 'bg-[#5f0f40]',
    textBackground: 'text-[#5f0f40]',
    textPrimary: 'text-white',
    textSecondary: 'text-gray-200',
  },
  Solid14: {
    background: 'bg-[#8338ec]',
    textBackground: 'text-[#8338ec]',
    textPrimary: 'text-white',
    textSecondary: 'text-gray-300',
  },
  Solid15: {
    background: 'bg-[#5a189a]',
    textBackground: 'text-[#5a189a]',
    textPrimary: 'text-white',
    textSecondary: 'text-gray-300',
  },
  Solid16: {
    background: 'bg-[#b14223]',
    textBackground: 'text-[#b14223]',
    textPrimary: 'text-white',
    textSecondary: 'text-gray-300',
  },
};

 
export const luxuryThemes = {
  Luxury1: {
    background: 'bg-[#4A3B2A]',  // Deep walnut brown
    textBackground: 'text-[#4A3B2A]',
    textPrimary: 'text-white',
    textSecondary: 'text-gray-300',
  },
  Luxury2: {
    background: 'bg-[#1D3C6D]',  // Professional navy blue
    textBackground: 'text-[#1D3C6D]',
    textPrimary: 'text-white',
    textSecondary: 'text-gray-300',
  },
  Luxury3: {
    background: 'bg-[#C7B0A2]',  // Soft muted beige
    textBackground: 'text-[#C7B0A2]',
    textPrimary: 'text-gray-800',
    textSecondary: 'text-gray-600',
  },
  Luxury4: {
    background: 'bg-[#6B4E8B]',  // Deep plum
    textBackground: 'text-[#6B4E8B]',
    textPrimary: 'text-white',
    textSecondary: 'text-gray-300',
  },
  Luxury5: {
    background: 'bg-[#3F2C20]',  // Dark chocolate
    textBackground: 'text-[#3F2C20]',
    textPrimary: 'text-white',
    textSecondary: 'text-gray-300',
  },
  Luxury6: {
    background: 'bg-[#4F4A6A]',  // Rich charcoal
    textBackground: 'text-[#4F4A6A]',
    textPrimary: 'text-white',
    textSecondary: 'text-gray-300',
  },
  Luxury7: {
    background: 'bg-[#7C4A4A]',  // Rich maroon
    textBackground: 'text-[#7C4A4A]',
    textPrimary: 'text-white',
    textSecondary: 'text-gray-300',
  },
  Luxury8: {
    background: 'bg-[#A27A00]',  // Golden ochre
    textBackground: 'text-[#A27A00]',
    textPrimary: 'text-white',
    textSecondary: 'text-gray-300',
  },
  Luxury9: {
    background: 'bg-[#4D4D2E]',  // Olive green
    textBackground: 'text-[#4D4D2E]',
    textPrimary: 'text-white',
    textSecondary: 'text-gray-300',
  },
  Luxury10: {
    background: 'bg-[#7A6E9C]',  // Soft lavender
    textBackground: 'text-[#7A6E9C]',
    textPrimary: 'text-white',
    textSecondary: 'text-gray-300',
  },
  Luxury11: {
    background: 'bg-[#2B2D42]',  // Dark slate gray
    textBackground: 'text-[#2B2D42]',
    textPrimary: 'text-white',
    textSecondary: 'text-gray-300',
  },
  Luxury12: {
    background: 'bg-[#3B3A36]',  // Deep charcoal gray
    textBackground: 'text-[#3B3A36]',
    textPrimary: 'text-white',
    textSecondary: 'text-gray-300',
  },
  Luxury13: {
    background: 'bg-[#2d6a4f]',
    textBackground: 'text-[#2d6a4f]',
    textPrimary: 'text-white',
    textSecondary: 'text-gray-300',
  },
  Luxury14: {
    background: 'bg-[#1b4332]',
    textBackground: 'text-[#1b4332]',
    textPrimary: 'text-slate-100',
    textSecondary: 'text-slate-300',
  },
  Luxury15: {
    background: 'bg-[#333533]',
    textBackground: 'text-[#333533]',
    textPrimary: 'text-white',
    textSecondary: 'text-gray-400',
  },
  Luxury16: {
    background: 'bg-[#274c77]',
    textBackground: 'text-[#274c77]',
    textPrimary: 'text-white',
    textSecondary: 'text-gray-300',
  },
  Luxury17: {
    background: 'bg-[#7d3417]',
    textBackground: 'text-[#7d3417]',
    textPrimary: 'text-white',
    textSecondary: 'text-gray-400',
  },
  Luxury18: {
    background: 'bg-[#41335c]',
    textBackground: 'text-[#41335c]',
    textPrimary: 'text-white',
    textSecondary: 'text-gray-400',
  },
  Luxury19: {
    background: 'bg-[#a67564]',
    textBackground: 'text-[#a67564]',
    textPrimary: 'text-white',
    textSecondary: 'text-gray-400',
  },
  Luxury20: {
    background: 'bg-[#723e31]',
    textBackground: 'text-[#723e31]',
    textPrimary: 'text-white',
    textSecondary: 'text-gray-300',
  },
  Luxury21: {
    background: 'bg-[#d88b79]',
    textBackground: 'text-[#d88b79]',
    textPrimary: 'text-gray-800',
    textSecondary: 'text-gray-600',
  },
  Luxury22: {
    background: 'bg-[#3A6D8C]',
    textBackground: 'text-[#3A6D8C]',
    textPrimary: 'text-white',
    textSecondary: 'text-gray-300',
  },
  Luxury23: {
    background: 'bg-[#507687]',
    textBackground: 'text-[#507687]',
    textPrimary: 'text-white',
    textSecondary: 'text-gray-300',
  },
  Luxury24: {
    background: 'bg-[#4a4e69]',
    textBackground: 'text-[#4a4e69]',
    textPrimary: 'text-white',
    textSecondary: 'text-gray-300',
  },
  Luxury25: {
    background: 'bg-[#184e77]',
    textBackground: 'text-[#184e77]',
    textPrimary: 'text-white',
    textSecondary: 'text-gray-200',
  },
  Luxury26: {
    background: 'bg-[#264653]',
    textBackground: 'text-[#264653]',
    textPrimary: 'text-white',
    textSecondary: 'text-gray-400',
  },
  Luxury27: {
    background: 'bg-[#b5838d]',
    textBackground: 'text-[#b5838d]',
    textPrimary: 'text-white',
    textSecondary: 'text-gray-300',
  },
  Luxury28: {
    background: 'bg-[#0d3b66]',
    textBackground: 'text-[#0d3b66]',
    textPrimary: 'text-white',
    textSecondary: 'text-gray-300',
  },
}

export const gradientThemes = {
  Gradient1: {
    background: 'bg-gradient-to-l from-blue-100 to-blue-300',
    textBackground: 'text-gradient-to-r from-blue-100 to-blue-300',
    textPrimary: 'text-gray-800',
    textSecondary: 'text-gray-600',
  },
  Gradient2: {
    background: 'bg-gradient-to-r from-gray-100 to-gray-300',
    textBackground: 'text-gradient-to-r from-gray-100 to-gray-300',
    textPrimary: 'text-gray-700',
    textSecondary: 'text-gray-500',
  },
  Gradient3: {
    background: 'bg-gradient-to-r from-green-100 to-teal-200',
    textBackground: 'text-gradient-to-r from-green-100 to-teal-200',
    textPrimary: 'text-gray-800',
    textSecondary: 'text-gray-600',
  },
  Gradient4: {
    background: 'bg-gradient-to-r from-purple-100 to-indigo-200',
    textBackground: 'text-gradient-to-r from-purple-100 to-indigo-200',
    textPrimary: 'text-gray-800',
    textSecondary: 'text-gray-600',
  },
  Gradient5: {
    background: 'bg-gradient-to-r from-teal-100 to-teal-300',
    textBackground: 'text-gradient-to-r from-teal-100 to-teal-300',
    textPrimary: 'text-gray-800',
    textSecondary: 'text-gray-600',
  },
  Gradient6: {
    background: 'bg-gradient-to-r from-slate-100 to-slate-200',
    textBackground: 'text-gradient-to-r from-slate-100 to-slate-200',
    textPrimary: 'text-gray-800',
    textSecondary: 'text-gray-600',
  },
  Gradient7: {
    background: 'bg-gradient-to-r from-yellow-100 to-orange-200',
    textBackground: 'text-gradient-to-r from-yellow-100 to-orange-200',
    textPrimary: 'text-gray-800',
    textSecondary: 'text-gray-600',
  },
  Gradient8: {
    background: 'bg-gradient-to-r from-emerald-100 to-emerald-300',
    textBackground: 'text-gradient-to-r from-emerald-100 to-emerald-300',
    textPrimary: 'text-gray-800',
    textSecondary: 'text-gray-600',
  },
  Gradient9: {
    background: 'bg-gradient-to-r from-indigo-100 to-indigo-300',
    textBackground: 'text-gradient-to-r from-indigo-100 to-indigo-300',
    textPrimary: 'text-gray-800',
    textSecondary: 'text-gray-600',
  },
  Gradient10: {
    background: 'bg-gradient-to-r from-rose-100 to-pink-200',
    textBackground: 'text-gradient-to-r from-rose-100 to-pink-200',
    textPrimary: 'text-gray-800',
    textSecondary: 'text-gray-600',
  },

  Gradient11: {
    background: 'bg-gradient-to-r from-gray-800 to-gray-900',
    textBackground: 'text-gradient-to-r from-gray-800 to-gray-900',
    textPrimary: 'text-gray-300',
    textSecondary: 'text-gray-400',
  },
  Gradient12: {
    background: 'bg-gradient-to-r from-slate-700 to-slate-900',
    textBackground: 'text-gradient-to-r from-slate-700 to-slate-900',
    textPrimary: 'text-slate-200',
    textSecondary: 'text-slate-400',
  },
  Gradient13: {
    background: 'bg-gradient-to-r from-indigo-900 to-gray-900',
    textBackground: 'text-gradient-to-r from-indigo-900 to-gray-900',
    textPrimary: 'text-indigo-300',
    textSecondary: 'text-indigo-400',
  },
  Gradient14: {
    background: 'bg-gradient-to-r from-purple-900 to-indigo-900',
    textBackground: 'text-gradient-to-r from-purple-900 to-indigo-900',
    textPrimary: 'text-gray-300',
    textSecondary: 'text-gray-400',
  },
  Gradient15: {
    background: 'bg-gradient-to-r from-teal-800 to-teal-900',
    textBackground: 'text-gradient-to-r from-teal-800 to-teal-900',
    textPrimary: 'text-teal-300',
    textSecondary: 'text-teal-400',
  },
  Gradient16: {
    background: 'bg-gradient-to-r from-blue-900 to-gray-800',
    textBackground: 'text-gradient-to-r from-blue-900 to-gray-800',
    textPrimary: 'text-blue-300',
    textSecondary: 'text-blue-400',
  },
  Gradient17: {
    background: 'bg-gradient-to-r from-emerald-900 to-teal-900',
    textBackground: 'text-gradient-to-r from-emerald-900 to-teal-900',
    textPrimary: 'text-gray-300',
    textSecondary: 'text-gray-400',
  },
  Gradient18: {
    background: 'bg-gradient-to-r from-black to-gray-800',
    textBackground: 'text-gradient-to-r from-black to-gray-800',
    textPrimary: 'text-gray-300',
    textSecondary: 'text-gray-400',
  },
  Gradient19: {
    background: 'bg-gradient-to-r from-red-900 to-gray-800',
    textBackground: 'text-gradient-to-r from-red-900 to-gray-800',
    textPrimary: 'text-red-300',
    textSecondary: 'text-gray-400',
  },
  Gradient20: {
    background: 'bg-gradient-to-r from-gray-700 to-black',
    textBackground: 'text-gradient-to-r from-gray-700 to-black',
    textPrimary: 'text-gray-200',
    textSecondary: 'text-gray-400',
  },

  Gradient21: {
    background: 'bg-gradient-to-r from-blue-200 to-blue-400',
    textBackground: 'text-gradient-to-r from-blue-200 to-blue-400',
    textPrimary: 'text-gray-800',
    textSecondary: 'text-blue-900',
  },
  Gradient22: {
    background: 'bg-gradient-to-r from-green-300 to-teal-500',
    textBackground: 'text-gradient-to-r from-green-300 to-teal-500',
    textPrimary: 'text-gray-800',
    textSecondary: 'text-teal-700',
  },

  Gradient23: {
    background: 'bg-gradient-to-r from-cyan-200 to-blue-500',
    textBackground: 'text-gradient-to-r from-cyan-200 to-blue-500',
    textPrimary: 'text-gray-900',
    textSecondary: 'text-blue-700',
  },
  Gradient24: {
    background: 'bg-gradient-to-r from-green-100 to-emerald-500',
    textBackground: 'text-gradient-to-r from-green-100 to-emerald-500',
    textPrimary: 'text-gray-800',
    textSecondary: 'text-emerald-700',
  },
  Gradient25: {
    background: 'bg-gradient-to-r from-blue-300 to-gray-200',
    textBackground: 'text-gradient-to-r from-blue-300 to-gray-200',
    textPrimary: 'text-gray-700',
    textSecondary: 'text-blue-600',
  },
  Gradient26: {
    background: 'bg-gradient-to-r from-yellow-300 to-gray-500',
    textBackground: 'text-gradient-to-r from-yellow-300 to-gray-500',
    textPrimary: 'text-gray-800',
    textSecondary: 'text-gray-700',
  },
  Gradient27: {
    background: 'bg-gradient-to-r from-gray-300 to-yellow-600',
    textBackground: 'text-gradient-to-r from-gray-300 to-yellow-600',
    textPrimary: 'text-gray-900',
    textSecondary: 'text-yellow-700',
  },
  Gradient28: {
    background: 'bg-gradient-to-r from-orange-400 to-gray-700',
    textBackground: 'text-gradient-to-r from-orange-400 to-gray-700',
    textPrimary: 'text-gray-900',
    textSecondary: 'text-gray-800',
  },

  Gradient29: {
    background: 'bg-gradient-to-r from-gray-200 to-blue-600',
    textBackground: 'text-gradient-to-r from-gray-200 to-blue-600',
    textPrimary: 'text-gray-900',
    textSecondary: 'text-blue-700',
  },
  Gradient30: {
    background: 'bg-gradient-to-r from-slate-300 to-indigo-600',
    textBackground: 'text-gradient-to-r from-slate-300 to-indigo-600',
    textPrimary: 'text-gray-900',
    textSecondary: 'text-indigo-700',
  },
  Gradient31: {
    background: 'bg-gradient-to-r from-teal-200 to-gray-600',
    textBackground: 'text-gradient-to-r from-teal-200 to-gray-600',
    textPrimary: 'text-gray-900',
    textSecondary: 'text-gray-800',
  },
  Gradient32: {
    background: 'bg-gradient-to-r from-pink-300 to-purple-600',
    textBackground: 'text-gradient-to-r from-pink-300 to-purple-600',
    textPrimary: 'text-white',
    textSecondary: 'text-gray-200',
  },
  Gradient33: {
    background: 'bg-gradient-to-r from-gray-400 to-gray-800',
    textBackground: 'text-gradient-to-r from-gray-400 to-gray-800',
    textPrimary: 'text-white',
    textSecondary: 'text-gray-300',
  },
  Gradient34: {
    background: 'bg-gradient-to-r from-purple-400 to-pink-500',
    textBackground: 'text-gradient-to-r from-purple-400 to-pink-500',
    textPrimary: 'text-gray-900',
    textSecondary: 'text-gray-800',
  },
  Gradient35: {
    background: 'bg-gradient-to-r from-[#2E4C6D] to-[#4A3B2A]', // Deep navy to dark walnut
    textBackground: 'text-[#2E4C6D]',
    textPrimary: 'text-white',
    textSecondary: 'text-gray-300',
  },
  Gradient36: {
    background: 'bg-gradient-to-r from-[#6B4E8B] to-[#2E2A36]', // Deep plum to charcoal
    textBackground: 'text-[#6B4E8B]',
    textPrimary: 'text-white',
    textSecondary: 'text-gray-300',
  },
  Gradient37: {
    background: 'bg-gradient-to-r from-[#3F2C20] to-[#A27A00]', // Dark chocolate to golden ochre
    textBackground: 'text-[#3F2C20]',
    textPrimary: 'text-white',
    textSecondary: 'text-gray-300',
  },
  Gradient38: {
    background: 'bg-gradient-to-r from-[#1D3C6D] to-[#2B2D42]', // Professional navy to slate gray
    textBackground: 'text-[#1D3C6D]',
    textPrimary: 'text-white',
    textSecondary: 'text-gray-300',
  },
  Gradient39: {
    background: 'bg-gradient-to-r from-[#4F4A6A] to-[#3B3A36]', // Rich charcoal to deep charcoal gray
    textBackground: 'text-[#4F4A6A]',
    textPrimary: 'text-white',
    textSecondary: 'text-gray-300',
  },
  Gradient40: {
    background: 'bg-gradient-to-r from-[#7C4A4A] to-[#4A3B2A]', // Rich maroon to dark walnut brown
    textBackground: 'text-[#7C4A4A]',
    textPrimary: 'text-white',
    textSecondary: 'text-gray-300',
  },
  Gradient41: {
    background: 'bg-gradient-to-r from-[#2E2A36] to-[#2B2D42]', // Dark charcoal to slate gray
    textBackground: 'text-[#2E2A36]',
    textPrimary: 'text-white',
    textSecondary: 'text-gray-300',
  },
  Gradient42: {
    background: 'bg-gradient-to-r from-[#4D4D2E] to-[#3B3A36]', // Olive green to deep charcoal
    textBackground: 'text-[#4D4D2E]',
    textPrimary: 'text-white',
    textSecondary: 'text-gray-300',
  },
  Gradient43: {
    background: 'bg-gradient-to-r from-[#5A3E36] to-[#3F2C20]', // Warm terracotta to dark chocolate
    textBackground: 'text-[#5A3E36]',
    textPrimary: 'text-white',
    textSecondary: 'text-gray-300',
  },
  Gradient44: {
    background: 'bg-gradient-to-r from-[#7A6E9C] to-[#2E4C6D]', // Soft lavender to rich navy
    textBackground: 'text-[#7A6E9C]',
    textPrimary: 'text-white',
    textSecondary: 'text-gray-300',
  },
};