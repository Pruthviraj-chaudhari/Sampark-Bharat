...
const ProfileSection = () => {
  ...
  return (
    <section id="profile" className="w-full py-4 md:pt-6 md:pb-24 lg:py-32 bg-slate-900">
      <div className="container grid grid-cols-1 lg:grid-cols-2 gap-8 px-4 md:px-6">
        ...
        <div className="flex flex-col items-start gap-6 px-4 md:px-0">
          {loading ? (
            ...
            <Balancer className="text-slate-200 text-3xl font-bold sm:text-4xl md:text-5xl">
              {profile?.name}
            </Balancer>
          )}
          {loading ? (
            ...
            <Balancer className="text-2xl text-slate-300 font-bold tracking-wider">
              {profile?.position}
            </Balancer>
          )}
          {loading ? (
            ...
            <Balancer className="text-slate-400 md:text-xl leading-relaxed">
              {profile?.description}
            </Balancer>
          )}
          <div className="flex flex-col md:flex-row items-start md:items-center gap-4">
            <div className="flex items-center gap-2">
              <MdLocationPin className="w-5 h-5 text-slate-600" />
              {loading ? (
                <Skeleton className="w-24 h-4 rounded" />
              ) : (
                <Balancer className="cursor-pointer text-slate-300 hover:text-muted transition-colors">
                  {profile?.location}
                </Balancer>
              )}
            </div>
            <div className="flex items-center gap-2">
              <IoCall className="w-5 h-5 text-slate-600" />
              {loading ? (
                <Skeleton className="w-24 h-4 rounded" />
              ) : (
                <Balancer className="cursor-pointer text-slate-300 hover:text-muted transition-colors">
            ...
            <div className="flex items-center gap-2">
              <IoMail className="w-5 h-5 text-slate-600" />
              {loading ? (
                <Skeleton className="w-24 h-4 rounded" />
              ) : (
                <Balancer className="cursor-pointer text-slate-300 hover:text-muted transition-colors">
